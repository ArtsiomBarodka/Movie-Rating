...
<header class="container-fluid">
    ...
    <div class="row justify-content-between text-white" id="header-navbar">
        <div class="col-2 align-self-center d-none d-md-block" id="header-project-name">
            <a href="/app/movies" class="text-white"><fmt:message key="app.title"/></a>
        </div>
        <nav class="col-xl-5 col-lg-6 col-md-7 col-sm-12 align-self-center" id="header-account">
            <div class="d-inline-flex">
                <a href="/app/movies" class="text-white"><fmt:message key="header.movie.list"/></a>
            </div>
            <div class="d-inline-flex">
                <a href="/app/movies/top" class="text-white"><fmt:message key="header.movie.top"/></a>
            </div>
        <c:choose>
            <c:when test="${not empty CURRENT_ACCOUNT_DETAILS}">
                <c:if test="${CURRENT_ACCOUNT_DETAILS.role == admin}">
                    <div class="d-inline-flex">
                        <a href="/app/movie/new/create" class="text-white"><fmt:message key="movie.create"/></a>
                    </div>
                </c:if>
                <c:if test="${CURRENT_ACCOUNT_DETAILS.role == user}">
                    <div class="d-inline-flex">
                        <a href="/app/user/${CURRENT_ACCOUNT_DETAILS.uid}" class="text-white">${CURRENT_ACCOUNT_DETAILS.name}</a>
                    </div>
                </c:if>
                <div class="d-inline-flex">
                    <a href="/app/logout" class="text-white"><fmt:message key="app.logout"/></a>
                </div>
            </c:when>
            <c:otherwise>
                <div class="d-inline-flex">
                    <a href="/app/show/sign-in" class="btn btn-light "><fmt:message key="header.sign-in"/></a>
                </div>
            </c:otherwise>
        </c:choose>
        </nav>
